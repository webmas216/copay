...
angular.module('copayApp.services').service('incomingData', function($log, $state, $timeout, $ionicHistory, bitcore, bitcoreCash, $rootScope, payproService, scannerService, appConfigService, popupService, gettextCatalog) {
  ...
  root.redir = function(data) {
    ...
    } else if (bitcore.URI.isValid(data.replace(/^poliscash:/,'polis:'))) {
        $log.debug('Handling poliscash URI with legacy address');
        var coin = 'bch';
        var parsed = new bitcore.URI(data.replace(/^poliscash:/,'polis:'));
