  ...
  root.init = function() {
    ...
    if (platformInfo.isChromeApp) {
      ...
      gui.App.on('open', function(pathData) {
        if (pathData.indexOf(/^polis(cash)?:/) != -1) {
          $log.debug('Bitcoin URL found');
          handleOpenURL({
            url: pathData.substring(pathData.indexOf(/^polis(cash)?:/))
          });
      ...
      if (navigator.registerProtocolHandler) {
        ...
        navigator.registerProtocolHandler('polis', url, 'Copay Bitcoin Handler');
        navigator.registerProtocolHandler('web+poliscash', url, 'Copay Bitcoin Cash Handler');
